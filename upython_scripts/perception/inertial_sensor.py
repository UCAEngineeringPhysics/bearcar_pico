...
class MPU6050:
    def __init__(self, i2c_channel=0, scl_id=9, sda_id=8, i2c_addr=0x68, dlpf_level=4):
        self.board_led = Pin(25, Pin.OUT)
        self.i2c = I2C(i2c_channel, scl=Pin(scl_id), sda=Pin(sda_id), freq=400_000)
    ...
    def read_data(self):
        ...
        return {
            "lin_acc_x": self.lin_acc_x,
            "lin_acc_y": self.lin_acc_y,
            "lin_acc_z": self.lin_acc_z,
...
if __name__ == "__main__":
    ...
    # SETUP
    sensor = MPU6050(i2c_channel=1, scl_id=15, sda_id=14, i2c_addr=0x68)

    # LOOP
    while True:
        data = sensor.read_data()
        # Logging, enable plotter
        print(f"[Pico time stamp: {ticks_ms()} ms]:")
        print(
            f"acc(m/s/s): x={data['lin_acc_x']:.4f}, y={data['lin_acc_y']:.4f}, z={data['lin_acc_z']:.4f}"
        )
        print(
            f"angv(rad/s): x={data['ang_vel_x']:.4f}, y={data['ang_vel_y']:.4f}, z={data['ang_vel_z']:.4f}"
        )
        sleep_ms(100)  # 10Hz
